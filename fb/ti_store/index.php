...
<html xmlns:fb="http://ogp.me/ns/fb#" lang="en">
  ...
  <script>
    window.fbAsyncInit = function() {
      FB.init({"appId": "533599820096656", "status": true, "cookie": true, "xfbml": true});
  ...
  <link href="//www.tomatoink.com/v/mp/css/ti-article.css" rel='stylesheet' type='text/css'/>
  <link href="facebook-shop.css" rel='stylesheet' type='text/css'/>
  </style>
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js"></script>
  </head>
  <body class="ti_cms_articleContent_area ti_fb-shop">
    <div class="ti_cms_row ti_fb-header">
      <div class="ti_cms_ten firstCol"><img src="//www.tomatoink.com/v/mp/facebook-shop/images/fb-header.png"><h1>Shop Ink &amp; Toner</h1></div>
      <div class="ti_cms_two lastCol">
       <p>
		<a href="//www.tomatoink.com/v/dc/yy/chat_page/tomatoink_chat.html" onclick="javascript:void window.open('//www.tomatoink.com/v/dc/yy/chat_page/tomatoink_chat.html','Comboink Chat Page','width=637,height=660,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=0,top=0');return false;"><span class="icon-bubbles"></span> Live Chat</a><br>
		<a href="mailto:customerservice@tomatoink.com?Subject=Help%20with%20the%20Facebook%20shop" target="_top"><span class="icon-envelope"></span> Email Us</a>
	   </p>
      </div>
    </div>
    <div class="ti_cms_row ti_fb-search">
      <div class="ti_cms_twelve ti_fb-searchContain">
        <form id="nxt-ac-form" name="form5" method="get" action="" onsubmit="return submitSearch5();">
          ...
          <input type="submit" border="0" name="imageField" value="Find Ink" class="ti_cms_redBtn" onclick="submitSearch5();"></input>
        </form>
      </div>
    </div>
...
if ($keywords != null) {
	//echo $keywords . "<br>";
	$sql = "SELECT productid, shorttitle, producturl, imageurl, price from ti_fbstore where longtitle2 like '%$keywords2%'";
}
else {
	//$sql = "SELECT productid, title, producturl, imageurl, price from fbstore limit $startID, 12";
	$sql = "SELECT productid, shorttitle, producturl, imageurl, price from ti_fbstore";
}
...
while($array = mysql_fetch_array($result)) {	
	echo '<div class="ti_fb-quarter">';
	echo '<div class="ti_fb-inner">';
	echo '<a target="_blank" href="//' . substr($array[2], 7) . '">';
	echo '<h2>' . $array[1] . '</h2>';
	echo '<div class="ti_fb-imgContain">'; 
	echo '<img src="//' . substr($array[3], 7) . '">';
	echo '</div>';
	echo '</a>';
	echo '<p><span class="ti_fb-price">$' .$array[4]. '</span>';
	echo '<a target="_blank" href="//' . substr($array[2], 7) . '">' . '<input type="submit" class="btn" value="Shop now"></input></a></p>';
	echo '</div>';
	echo '</div>';
	//$count++;
	//if ($count>12) break;
...
?>	
      ...
      <div class="ti_cms_row">
        <div class="ti_cms_twelve ti_fb-footer"></div>
      </div>
    </div>
  </body>
<script>
    ...
    function submitSearch5() {  //need to change as the form function name if you add more
        if (document.getElementById("search_v5_5").value != "") {   //need to change as the id in form input if you add more
			...
			var searchUrl = "https://secure.compandsave.com/lf/ti_store/index.php?keywords=";
...
$(document).ready(function(){
	...
	currentURL = "https://secure.compandsave.com/lf/ti_store/index.php";
	keywords = '<?=isset($_GET['keywords']) ? $_GET['keywords'] : null?>';
	...
	document.getElementById('ti_fb-currentPage').innerHTML = page;
