...
<!--
For development:
	.cas_surRe_coupon - replace contents with unique code
	.cas_surRe_productImg - replace with product images
	.cas_surRe_price - replace with product prices (after 25% discount)
	.cas_surRe_productName - replace with product names
	.cas_surRe_retail - replace with retail prices
	.cas_surRe_printerModel - replace with printer model, default text is ok as is
	.cas_surRe_printerBrand - replace with printer brand, default is empty
...
<div class='cas_cms_articleContent_area cas_surRe_container'>
	<div class='cas_cms_row'>
		<div class='cas_cms_twelve'>
			<h1>Hi <%=firstname%>, your exclusive 25% off coupon is:<br><span class='cas_surRe_coupon'><%=couponcode%></span></h1>
			<h3>Use at checkout to activate your discounted prices shown below, but only until 7/28/2014!</h3>
		</div>
	</div>
	<div class='cas_cms_row cas_surRe_topProduct'>
		<div class='cas_cms_four firstCol'>
			<a target='_blank' href='//www.compandsave.com/ProductDetails.asp?ProductCode=<% Response.write(product) %>'>
				<img src='//www.compandsave.com/v/vspfiles/photos/<% Response.write(product) %>-2.jpg' onerror="this.src='/www.compandsave.com/v/vspfiles/templates/10/images/nophoto.gif';" class='cas_surRe_productImg' alt='This product works with your printer model.'>
			</a>
		</div>
		<div class='cas_cms_eight lastCol'>
			<a target='_blank' href='//www.compandsave.com/ProductDetails.asp?ProductCode=<% Response.write(product) %>'>
				<h2 class='cas_surRe_productName'><% Response.write(ProductName) %></h2>
			</a>
			<div class='cas_cms_row'>
				<div class='cas_cms_seven firstCol'>
					<h2 class='cas_surRe_dollar'>Price after coupon: <sup>$</sup><span class='cas_surRe_price'><% Response.write(ProductPrice) %></span></h2>
					<p>Retail price: <sup>$</sup><span class='cas_surRe_retail'><% Response.write(ListPrice) %></span></p>
					<p><label><span>Qty: </span><input type='text' value='1' maxlength='3' class='cas_surRe_qty' id='cas_qty_0'></input></label>
				...
				<div class='cas_cms_five lastCol'>
					<ul class='cas_surRe_checklist'>
						<li><span class='icon-checkmark cas_surRe_check'></span><span class='cas_surRe_check'>Use code <span class='cas_surRe_coupon'><%=couponcode%></span> to get your discounted price</span></li>
						<li><span class='icon-checkmark cas_surRe_check'></span><span class='cas_surRe_check'>One year money back guarantee</span></li>
						<li><span class='icon-checkmark cas_surRe_check'></span><span class='cas_surRe_check'>Ships in 24 hours!</span></li>
					</ul>
				</div>
			</div>
		</div>
		...
		<div class='cas_cms_twelve jcarousel-wrapper'>
			<div class='jcarousel'>
				<ul>
					...
					<li>
				        <a target='_blank' href='//www.compandsave.com/ProductDetails.asp?ProductCode=<%Response.write(single_node)%>'><h3 class='cas_surRe_productName'><% Response.write(ProductName) %></h3>
				        <img src='//www.compandsave.com/v/vspfiles/photos/<%Response.write(single_node)%>-2.jpg' onerror="this.src='/www.compandsave.com/v/vspfiles/templates/10/images/nophoto.gif';" class='cas_surRe_productImg'></a>
				        <p class='cas_surRe_dollar'>Price after coupon: <sup>$</sup><span class='cas_surRe_price'><% Response.write(ProductPrice) %></span></p>
						<p>Retail price: <sup>$</sup><span class='cas_surRe_retail'><% Response.write(ListPrice) %></span></p>
				        <label><span>Qty: </span><input type='text' id='cas_qty_<%=count%>' value='1' maxlength='3' class='cas_surRe_qty'></input></label>
	    	...
	    	<% end if %>
	    	<% if count > 0 then %>
	    	<h2>The products above are compatible with your <strong><span class='cas_surRe_printerBrand'><%=brandname%></span> <span class='cas_surRe_printerModel'>printer model</span></strong>.</h2>
	    	<% end if %>
		</div>
	</div>
	<div class='cas_cms_row'>
		<div class='cas_cms_twelve cas_surRe_disclaim'>
			<div class='cas_cms_divider'></div>
			<p>CompAndSave.com is proud to offer wallet friendly prices on high quality ink cartridges for your <span class='cas_surRe_printerModel'>printer model</span>. We specialize in providing ink cartridges for any <span class='cas_surRe_printerBrand'><%=brandname%></span> printer. Our ink cartridges are diligently re-engineered at our manufacturing facility to ensure the same print quality as brand name cartridges. Our high print quality and low prices are backed with a <strong>1 year money back guarantee</strong>, as well as our <strong>99% customer satisfaction rate</strong>!</p>
			<p>Using compatible or remanufactured printer cartridges will never void your printer warranty.  Feel free to contact us at <a href="mailto:customercare@compandsave.com">customercare@compandsave.com</a> if you have any further questions or if you need help finding your replacement cartridges from the compatible <span class='cas_surRe_printerModel'>printer model</span> ink cartridges above. Hurry and take advantage of our great prices now!</p>
			<p><strong>Disclaimer:</strong> This offer cannot be combined with any other offer or coupon code and cannot be applied to past purchases. Coupon must be applied at checkout to receive your discount. Other restrictions may apply. Each coupon code may only be used once. Coupon expires 7/28/2014 11:59 PST.</p>
		</div>
	</div>
</div>
...
jQuery('.cas_surRe_printerModel').html(printername);
}
...
<div class='cas_cms_articleContent_area cas_surRe_container'>
	<div class='cas_cms_row'>
		...
		<div class='cas_cms_eight'>
			...
			<p class='cas_cms_alignLeft'>There seems to be an issue processing your request for your 25% off coupon. Please send us an email and we will help you resolve the problem. Please include your name, email address, and any other details you think we may need to know.</p>
