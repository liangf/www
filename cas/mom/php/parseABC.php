...
/*
// down the msm-abc entries from msm_abc table
$file = fopen("../data/abc-msm-new.csv","w");

$sql = "SELECT * FROM `msm_abc` WHERE formID=2";
...
// update the recent record to msm_abc table
$sql_abc = "SELECT max(time) as maxtime FROM msm_abc";
$result = mysql_query($sql_abc);
$array = mysql_fetch_array($result);
echo $array['maxtime'] . "<br>";
$maxtime_abc = $array['maxtime'];

$sql = "SELECT * FROM `msm_form` WHERE formID=2";
$result = mysql_query($sql);
$array = mysql_fetch_array($result);
while ($array = mysql_fetch_array($result)) {
	if ($array[8] > $maxtime_abc) {
		echo $array[0] . " ";
		echo $array[1] . " ";
		echo $array[2] . " ";
		echo $array[3] . " ";
		...
		echo $array[8] . " ";
		echo $array[9] . "<br>";
		$sql_new = "INSERT INTO `msm_abc` (id, firstname, lastname, address, city, zip, state, email, time, formID) 
