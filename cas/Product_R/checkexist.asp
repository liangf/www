...
Set OrderObj = Collector.RetrievePt(pcode)
...
Class OrderTrackingCollector
	...
	Public Function RetrievePt(ByVal pccan) 'As ParentCategory
		Dim SQL 'As String
		...
		SQL = VolusionAPICallObject.ReadFile( _
			FSO.BuildPath(mInstallPath, CONFIGURATION_CP_SQL_FILE) _
		)
		
		cp = mVBExt.OptimizedReplaceAll(cp,"'","")
		SQL = mVBExt.OptimizedReplaceAll(SQL, "#{cp}", cp)
		
		'Make custom API call
		Call VolusionAPICallObject.DoCustomAPICall( _
			SQL, _
			VolusionAPICallObject.ReadFile( _
			FSO.BuildPath(mInstallPath, CONFIGURATION_CP_XSD_FILE) _
			) _
		)
