...
Class OrderTrackingCollector
    ...
    Public Function RetrieveMonth(Byval pc, Byval dif) 
		Dim SQL 
		...
		SQL = VolusionAPICallObject.ReadFile( _
			FSO.BuildPath(mInstallPath, CONFIGURATION_P2_SQL_FILE) _
		)
		
		 productcode = mVBExt.OptimizedReplaceAll(productcode,"'","")
		 diff = mVBExt.OptimizedReplaceAll(diff,"'","")
		 SQL = mVBExt.OptimizedReplaceAll(SQL, "#{productcode}", productcode)
		 SQL = mVBExt.OptimizedReplaceAll(SQL, "#{diff}", diff)
