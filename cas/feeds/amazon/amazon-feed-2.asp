...
For Each ProductItem In OrderObj.Products 
	...
	For i = 2 To L
		...
		'bullet_point(i) = tmp
	Next
	bullet_point(1) = "One year money back guarantee!"
	bullet_point(2) = "Shop now to save an extra 10%"
	...
	br = tmpArray(0)
	If br = "INK" OR br="WINK" OR br="ZINK" Then
		bullet_point(4) = "(New) Compatible ink cartridge"
	ElseIf br="TONER" OR br="WTONER" OR br="ZTONER" Then
		bullet_point(4) = "(New) Compatible toner cartridge"
	ElseIf br = "DRUM" Then
		bullet_point(4) = "(New) Compatible drum unit"
	ElseIf br = "RIBBON" Then
		bullet_point(4) = "(New) Compatible ribbon"		
	ElseIf br = "PH" Then
		bullet_point(4) = "New photo paper...(or whatever it is)"		
	ElseIf br = "SPEC" Then
		If Instr(Lcase(ProductItem.title), "ink")>0 Then
			bullet_point(4) = "(New) Compatible ink cartridge"
		ElseIf Instr(Lcase(ProductItem.title), "toner")>0 Then
			bullet_point(4) = "(New) Compatible toner cartridge"
		Else 
			bullet_point(4) = "(New) Compatible ink cartridge (or whatever it is)"
		End If
	Else 
		bullet_point(4) = "(New) Compatible ink cartridge (or whatever it is)"
	End If
	...
	brand = "CompAndSave"
	f.Writeline (amazon_category & Tab & ProductItem.title & Tab & link & Tab & ProductItem.id & Tab & ProductItem.price & Tab & image_link & Tab & upc & Tab & brand & Tab & recommended_browse_node & Tab & ProductItem.description & Tab & manufacturer & Tab & mpn & Tab & shipping_cost & Tab & model_number & Tab & weight & Tab & bullet_point(1) & Tab & bullet_point(2) & Tab & bullet_point(3) & Tab & bullet_point(4) & Tab & bullet_point(5) & Tab & keywords(1) & Tab & keywords(2) & Tab & keywords(3) & Tab & keywords(4))
