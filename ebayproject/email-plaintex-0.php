...
while($array = mysql_fetch_array($result)) {
	...
	if ( mysql_num_rows(mysql_query($sql2))<1 ) {
		$message .= $array['customlabel'] . "\t";
		$message .= $array['costprice'] . "\t";
		$message .= $array['shippingcost'] . "\t";
		$message .= $array['time'] . "\n";
	}
}
$message .=	"\n\n";
...
while($array = mysql_fetch_array($result)) {	
	if ($array['morethan5']=="N") {
		$message .= $array["customlabel"] . "\t";
		$message .= strval($array["price"]) . "\t";
		$message .= strval($array["costprice"]) . "\t";
		$message .= strval($array["shippingcost"]) . "\t";
		$message .= strval($array["fvf"]) . "\t";
		$message .= strval($array["jl5"]) . "\t";
		$message .= strval($array["paypalfee"]) . "\t";
		$message .= strval($array["netprofit"]) . "\t";
