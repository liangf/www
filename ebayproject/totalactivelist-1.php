...
function deleterow()
{
document.getElementById("tab-activelist2").deleteRow(1);
}

function insertrow(colum1, colum2, colum3, colum4)
{
    ...
    for (var i = 0; i < rows.length; i++) {
        rows[i].onclick = (function() {
            ...
            return function() {
				...
				for (var i=1; i<rowCount; i++) deleterow();
				...
				jQuery.post("http://www.casinkadmin.com/liang/ebayproject/getRow.php",
					...
					function(data){ 
						...
						for (var i=0; i<items.length-1; i++) {
							$row2 = items[i].split("#n#");
							//alert($row2);
							$customlabel = $row2[0];
							$costprice = $row2[1];
							$shippingcost = $row2[2];
							$time = $row2[3];
							insertrow($customlabel, $costprice, $shippingcost, $time);
						}
						//alert(users.length);
						//document.getElementById('h2-newebay').innerHTML="Ebay Active List Updated!";
					}
				);	
