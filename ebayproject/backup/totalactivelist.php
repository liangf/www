...
if ($_POST["btnmerge"] || $_GET["sortname"]=="label_m" || $_GET["sortname"]=="price_m") {
...
function showActiveList() {
	...
	if ($row>=1) {
		echo '<div id="wrapper">';
		echo '<table id="tab-activelist" class="inlineTable">';
		echo '<tr>';
		...
		echo '<table id="tab-activelist2" class="inlineTable" style="display:none">';
		//echo '<tr>';
...
function showMerge() {	
	...
	if ($_GET["sortname"]=="label_m" && $_GET["sort"] == "desc") {
		...
		$sql = "SELECT a.customlabel, t.price, a.costprice, a.shippingcost FROM ebay_tmpa as a inner join ebay_tmpt as t on a.customlabel2=t.customlabel2 order by a.customlabel DESC";
	}
	if ($_GET["sortname"]=="label_m" && $_GET["sort"] == "asc") {
		$href_label = "//www.casinkadmin.com/liang/ebayproject/totalactivelist.php?sortname=label_m&sort=desc";
		$sql = "SELECT a.customlabel, t.price, a.costprice, a.shippingcost FROM ebay_tmpa as a inner join ebay_tmpt as t on a.customlabel2=t.customlabel2 order by a.customlabel ASC";
	}
	...
	if ($_GET["sortname"]=="price_m" && $_GET["sort"] == "desc") {
		...
		$sql = "SELECT a.customlabel, t.price, a.costprice, a.shippingcost FROM ebay_tmpa as a inner join ebay_tmpt as t on a.customlabel2=t.customlabel2 order by t.price DESC";
	}
	if ($_GET["sortname"]=="price_m" && $_GET["sort"] == "asc") {
		$href_price = "//www.casinkadmin.com/liang/ebayproject/totalactivelist.php?sortname=price_m&sort=desc";
		$sql = "SELECT a.customlabel, t.price, a.costprice, a.shippingcost FROM ebay_tmpa as a inner join ebay_tmpt as t on a.customlabel2=t.customlabel2 order by t.price ASC";
	}	
	...
	if ($row>=1) {
		...
		echo '<td style="width:150px"><b>Cost price</td>';
		echo '<td style="width:150px"><b>Shipping cost</td>';
		echo '<td style="width:150px"><b>Ebay FVF</td>';
		echo '<td style="width:150px"><b>JL 5% commission</td>';
		echo '<td style="width:150px"><b>Paypal fee</td>';
		echo '<td style="width:150px"><b>Net Profit</td>';
		echo '<td style="width:150px"><b>More than $5 Profit</td>';
		echo '</tr>';
		while($array = mysql_fetch_array($result)) {
			...
			echo "<td>" . $array['costprice'] . "</td>";
			echo "<td>" . $array['shippingcost'] . "</td>";
			echo "<td>" . $array['price']*0.06 . "</td>";
			echo "<td>" . $array['price']*0.05 . "</td>";
			echo "<td>" . ($array['price']*0.025+0.3) . "</td>";
			echo "<td>" . $net_profit . "</td>";
			echo "<td>" . $more_than_5 . "</td>";
			echo "</tr>";	
			...
			// insert the distinct data into ebay_tmpa
			$sql = "insert into ebay_profit (id, customlabel, price, costprice, shippingcost, fvf, jl5, paypalfee, netprofit, morethan5, time) values(0, '$customlabel', $price, $costprice, $shippingcost, $fvf, $jl5, $paypalfee, $netprofit, '$morethan5', now())";
			if( !mysql_query($sql) ) echo "'" .$customlabel ."'" ." added no success. <br>";				
		}
		echo "</table>";
	} else { echo "No data in DB.<br>";}	
