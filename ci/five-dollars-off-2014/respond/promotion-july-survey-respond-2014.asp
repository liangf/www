...
Public Function ExtractProduct(ByVal ProductCode) 'As ParentCategory
		...
		if ProductCode <> "" then 
			'Setup SQL using template
			SQL = "SELECT p.ProductName, vMerchant.ufn_Get_Virtual_Columns(pj.ProductID, pj.ProductCode, 'Accessories') AS Accessories, pe.ProductPrice, pe.ListPrice FROM Products p INNER JOIN Products_Extended pe On p.ProductID = pe.ProductID INNER JOIN Products_Joined pj ON p.ProductID = pj.ProductID WHERE p.ProductCode ='" & ProductCode & "';" 

			'"SELECT Products_Joined.ProductCode, vMerchant.ufn_Get_Virtual_Columns(Products_Joined.ProductID, Products_Joined.ProductCode, 'Accessories') AS Accessories FROM Products_Joined WITH (NOLOCK) ORDER BY Products_Joined.ProductCode"
...
If Not Request.QueryString("printer")= Null Or Not Request.QueryString("printer") = "" Then
printer = Request.QueryString("printer")
Else
printer = ""
End If
...
if printer <> "" and printer <> "0" then 

result = Collector.ExtractPrinter(printer)
RelatedProducts = ""
product_info = Split(result, "@@@")
if Ubound(product_info) > 3 then 
product = product_info(0)
ProductName = product_info(1)
ProductPrice = round(product_info(2)*0.75,2)
ListPrice = round(product_info(3),2) 
RelatedProducts = product_info(4) 
end if 


end if

if (product_temp <> "") OR (printer <> "" and printer <> "0" and product = "") then 
...
<!--
For development:
	.ci_surRe_coupon - replace contents with unique code
	.ci_surRe_productImg - replace with product images
	.ci_surRe_price - replace with product prices (after 25% discount)
	.ci_surRe_productName - replace with product names
	.ci_surRe_retail - replace with retail prices
	.ci_surRe_printerModel - replace with printer model, default text is ok as is
	.ci_surRe_printerBrand - replace with printer brand, default is empty
...
<link href='/v/mp/css/ci-article.css' rel='stylesheet' type='text/css'>
<link href='css/2014-july-survey-respond-ci.css' rel='stylesheet' type='text/css'>
...
<!-- Set Cookie !-->
<script type="text/javascript">document.cookie ="couponcode=<%=couponcode%>; expires=Mon, 28 July 2014 23:59:59 UTC; path=/";</script>

<div class='ci_cms_articleContent_area ci_surRe_container'>
	<div class='ci_cms_row'>
		<div class='ci_cms_twelve'>
			<h1>Hi <%=firstname%>, your exclusive 25% off coupon is:<br><span class='ci_surRe_coupon'><%=couponcode%></span></h1>
			<h3>Use at checkout to activate your discounted prices shown below, but only until 7/28/2014!</h3>
		</div>
	</div>
	<div class='ci_cms_row ci_surRe_topProduct'>
		<div class='ci_cms_four firstCol'>
			<a target='_blank' href='//www.comboink.com/ProductDetails.asp?ProductCode=<% Response.write(product) %>'>
				<img src='//www.comboink.com/v/vspfiles/photos/<% Response.write(product) %>-2.jpg' onerror="this.src='/v/vspfiles/templates/10/images/nophoto.gif';" class='ci_surRe_productImg' alt='This product works with your printer model.'>
			</a>
		</div>
		<div class='ci_cms_eight lastCol'>
			<a target='_blank' href='//www.comboink.com/ProductDetails.asp?ProductCode=<% Response.write(product) %>'>
				<h2 class='ci_surRe_productName'><% Response.write(ProductName) %></h2>
			</a>
			<div class='ci_cms_row'>
				<div class='ci_cms_seven firstCol'>
					<h2 class='ci_surRe_dollar'>Price after coupon: <sup>$</sup><span class='ci_surRe_price'><% Response.write(ProductPrice) %></span></h2>
					<p>Retail price: <sup>$</sup><span class='ci_surRe_retail'><% Response.write(ListPrice) %></span></p>
					<p><label><span>Qty: </span><input type='text' value='1' maxlength='3' class='ci_surRe_qty' id='ci_qty_0'></input></label>
					<input type="hidden" id='ci_productcode_0' value='<% Response.write(product) %>'>
					<input class='btn ci_cms_redBtn' type='button' onclick='addtocart(0)' value='Add To Cart'></input></p>
				</div>
				<div class='ci_cms_five lastCol'>
					<ul class='ci_surRe_checklist'>
						<li><span class='icon-checkmark ci_surRe_check'></span><span class='ci_surRe_check'>Use code <span class='ci_surRe_coupon'><%=couponcode%></span> to get your discounted price</span></li>
						<li><span class='icon-checkmark ci_surRe_check'></span><span class='ci_surRe_check'>One year money back guarantee</span></li>
						<li><span class='icon-checkmark ci_surRe_check'></span><span class='ci_surRe_check'>Ships in 24 hours!</span></li>
					</ul>
				</div>
			</div>
		</div>
	...
	<div class='ci_cms_row'>
		<div class='ci_cms_twelve jcarousel-wrapper'>
			<div class='jcarousel'>
				<ul>
					<% 
					...
					<li>
				        <a target='_blank' href='//www.comboink.com/ProductDetails.asp?ProductCode=<%Response.write(single_node)%>'><h3 class='ci_surRe_productName'><% Response.write(ProductName) %></h3>
				        <img src='//www.comboink.com/v/vspfiles/photos/<%Response.write(single_node)%>-2.jpg' onerror="this.src='/v/vspfiles/templates/10/images/nophoto.gif';" class='ci_surRe_productImg'></a>
				        <p class='ci_surRe_dollar'>Price after coupon: <sup>$</sup><span class='ci_surRe_price'><% Response.write(ProductPrice) %></span></p>
						<p>Retail price: <sup>$</sup><span class='ci_surRe_retail'><% Response.write(ListPrice) %></span></p>
				        <label><span>Qty: </span><input type='text' id='ci_qty_<%=count%>' value='1' maxlength='3' class='ci_surRe_qty'></input></label>
				        <input type="hidden" id="ci_productcode_<%=count%>" value='<%=single_node%>'>
				        <input type='button' onclick='addtocart(<%=count%>)' value='Add to cart' class='ci_cms_redBtn'></input>
					</li>
			...
			<div class='jcarousel-controls'>
				<a href='#' class='jcarousel-control-prev'><span class='icon-arrow-left2'></span></a>
	    		<a href='#' class='jcarousel-control-next'><span class='icon-arrow-right2'></span></a>
	    	</div>
	    	<% end if %>
	    	<% if count > 0 then %>
	    	<h2>The products above are compatible with your <strong><span class='ci_surRe_printerBrand'><%=brandname%></span> <span class='ci_surRe_printerModel'>printer model</span></strong>.</h2>
	    	<% end if %>
		</div>
	</div>
	<div class='ci_cms_row'>
		<div class='ci_cms_twelve ci_surRe_disclaim'>
			<div class='ci_cms_divider'></div>
			<p>ComboInk.com is proud to offer wallet friendly prices on high quality ink cartridges for your <span class='ci_surRe_printerModel'>printer model</span>. We specialize in providing ink cartridges for any <span class='ci_surRe_printerBrand'><%=brandname%></span> printer. Our ink cartridges are diligently re-engineered at our manufacturing facility to ensure the same print quality as brand name cartridges. Our high print quality and low prices are backed with a <strong>1 year money back guarantee</strong>, as well as our <strong>99% customer satisfaction rate</strong>!</p>
			<p>Using compatible or remanufactured printer cartridges will never void your printer warranty.  Feel free to contact us at <a href="mailto:customercare@comboink.com">customercare@comboink.com</a> if you have any further questions or if you need help finding your replacement cartridges from the compatible <span class='ci_surRe_printerModel'>printer model</span> ink cartridges above. Hurry and take advantage of our great prices now!</p>
			<p><strong>Disclaimer:</strong> This offer cannot be combined with any other offer or coupon code and cannot be applied to past purchases. Coupon must be applied at checkout to receive your discount. Other restrictions may apply. Each coupon code may only be used once. Coupon expires 7/28/2014 11:59 PM <span title='Pacific Standard Time' class='ci_surRe_help'>PST</span>.</p>
		</div>
	</div>
</div>
...
function addtocart(id) {
	var qty_id = "ci_qty_" + id;
	var code_id = "ci_productcode_" + id;
	var qty = document.getElementById(qty_id).value;
	var code = document.getElementById(code_id).value;
	window.location = "//www.comboink.com/shoppingcart.asp?ProductCode=" + code + "&QTY." + code + "=" + qty;

}
if (typeof printername != "undefined") {
jQuery('.ci_surRe_printerModel').html(printername);
}
...
<div class='ci_cms_articleContent_area ci_surRe_container'>
	<div class='ci_cms_row'>
		<div class='ci_cms_twelve'>
			...
			<p class='ci_cms_alignLeft'>There seems to be an issue processing your request for your 25% off coupon. Please send us an email and we will help you resolve the problem. Please include your name, email address, and any other details you think we may need to know.</p>
			<p class='ci_cms_centerTxt'><a class='btn ci_cms_redBtn' href='mailto:mehmeta%40compandsave.com?subject=Help%20with%20my%2025%25%20off%20coupon&body=Name%3A%0AEmail%20address%3A'>Email us</a></p>
		</div>
	</div>
</div>
<!-- #include virtual="/v/vspfiles/templates/comboink/footer_static.html" -->
