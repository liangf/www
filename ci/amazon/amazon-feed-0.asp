...
For Each ProductItem In OrderObj.Products 
	...
	L= UBound(tmpArray)
	For i = 0 To L
		bullet_point(i) = tmpArray(i)
		If Len(bullet_point(i)) Then
			'tmp = Mid(tmp, 1, InStr(tmp,"</li>")-1)
			bullet_point(i) = Mid(bullet_point(i), 1, Len(bullet_point(i))-5)
			'Response.write(bullet_point(i) & ", ")
		End If
		'bullet_point(i) = tmp
	Next
	'Response.write("<br>")
	bullet_point(4) = "Free shipping for orders above $50"
	
	keywords(1) = brand
	keywords(2) = mpn
	
	
	f.Writeline (amazon_category & Tab & ProductItem.title & Tab & link & Tab & ProductItem.id & Tab & ProductItem.price & Tab & image_link & Tab & upc & Tab & brand & Tab & recommended_browse_node & Tab & ProductItem.description & Tab & manufacturer & Tab & mpn & Tab & shipping_cost & Tab & model_number & Tab & weight & Tab & bullet_point(1) & Tab & bullet_point(2) & Tab & bullet_point(3) & Tab & bullet_point(4) & Tab & keywords(1) & Tab & keywords(2) & Tab & keywords(3) & Tab & keywords(4))
