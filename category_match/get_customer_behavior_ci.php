...
$file = fopen("Brand_Specific_CI.csv", "w");


$count = 0;
$array_email = array();
$sql = "SELECT * FROM july_survey2 where FormID=2";
...
echo "july_survey2" ." => " ."ci: " .$count . "<br>";




$array_email2 = array();
$sql2 = "SELECT * FROM customer_behavior_ci";
...
while ($array = mysql_fetch_array($result)) {
	...
	if ($array_email[$email] > 0) {
		...
		if ( !$array_email2[$email] ) {
			...
			if ($productprice>0 && $productprice < $old_productprice) {
...
echo "customer_behavior_ci" ." AFTER SUBTRACTION: " .count($array_email2)  ."<br>";


$count = 0;
$count2 = 0;
$sql3 = "SELECT * FROM customer_behavior_ci";
...
echo "ci write: " .$count . "/" . ($count+$count2) ."<br>";
echo "<br><br>";
